...
{ 
  ...
  else
  {
    ...
    else
    {
      ...
      if((SD_state != MSD_OK) || (BSP_SD_GetCardState() != SD_TRANSFER_OK))
      {
      ...
      else
      {
        ...
        SD_state = BSP_SD_WriteBlocks((uint32_t *)aTxBuffer, BLOCK_START_ADDR, NUM_OF_BLOCKS, SDMMC_DATATIMEOUT);
        ...
        else
        {
          ...
          SD_state = BSP_SD_ReadBlocks((uint32_t *)aRxBuffer, BLOCK_START_ADDR, NUM_OF_BLOCKS, SDMMC_DATATIMEOUT);
          if(SD_state != MSD_OK)
          {
